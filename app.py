...
if 'HONEY_BADGER_KEY_IVIEW_RSS_MAXSTUFF_NET' in os.environ:
    ...
    honeybadger.configure(
        api_key=os.environ['HONEY_BADGER_KEY_IVIEW_RSS_MAXSTUFF_NET'])
else:
        ...
        if show_info['type'] == 'series':
            for series in show_info['_embedded']['seriesList']:
                current_series = json.loads(
                    requests.get(
                        "https://api.iview.abc.net.au/v2" +
                        series['_links']['deeplink']['href']
                    ).content.decode()
                )
                for episode in current_series['_embedded']['selectedSeries']['_embedded']['videoEpisodes']:
                    fe = fg.add_entry()
                    fe.id("https://iview.abc.net.au" +
                    ...
                    epiry_date = datetime.datetime.strptime(
                        episode['expireDate'], "%Y-%m-%d %H:%M:%S").strftime("%I:%M%p %d/%m/%Y")
                    fe.description(
                        f"Expires {epiry_date}\n{episode['description']}")
                    fe.link(href=episode['shareUrl'])
                    fe.published(datetime.datetime.strptime(
                        episode['pubDate'], "%Y-%m-%d %H:%M:%S").astimezone())
            ...
            honeybadger.notify(
                f"{show_info['type']} show type is not recognised!", context={"show_id": show})
            return "501", 501
    elif request_result.status_code == 404:
        return "404", 404
    else:
        honeybadger.notify(
            f"{request_result.status_code} error code is not recognised!", context={"show_id": show})
        return "501", 501
...
def watchlist(uid):
    ...
    watchlist_resp = requests.get(
        "https://api.seesaw.abc.net.au/v1/saved/watchlist/show?source=iview&slug=watchlist&raw=1&done=0&UID="+uid)
    if watchlist_resp.status_code == 200:
        ...
        fg.author({'name': 'Max', 'uri': 'https://maxstuff.net'})
        fg.description(
            "Compiled episodes of all shows on your iView watchlist.")
        for show_dict in watchlist['data']:
            ...
            if request_result.status_code == 200:
                ...
                if show_info['type'] == 'series':
                    if "unavailableMessage" not in show_info:
                        for series in show_info['_embedded']['seriesList']:
                            ...
                            for episode in current_series['_embedded']['selectedSeries']['_embedded']['videoEpisodes']:
                                ...
                                # series_text = "S"+current_series['_links']['selectedSeries']['id']+" "
                                fe.title(
                                    f'{show_info["title"]} - {series_text}{episode["title"]}')
                                epiry_date = datetime.datetime.strptime(
                                    episode['expireDate'], "%Y-%m-%d %H:%M:%S").strftime("%I:%M%p %d/%m/%Y")
                                fe.description(
                                    f"Expires {epiry_date}\n{episode['description']}")
                            ...
                            if include_extras and 'videoExtras' in current_series['_embedded']['selectedSeries']['_embedded']:
                                for episode in current_series['_embedded']['selectedSeries']['_embedded']['videoExtras']:
                                    ...
                                    # series_text = "S"+current_series['_links']['selectedSeries']['id']+" "
                                    fe.title(
                                        f'{series_text}EXTRA {episode["title"]}')
                                    epiry_date = datetime.datetime.strptime(
                                        episode['expireDate'], "%Y-%m-%d %H:%M:%S").strftime("%I:%M%p %d/%m/%Y")
                                    fe.description(
                                        f"Expires {epiry_date}\n{episode['description']}")
                    ...
                    honeybadger.notify(f"{show_info['type']} show type is not recognised!", context={
                                       "show_id": show, "uid": uid})
            else:
                honeybadger.notify(f"{request_result.status_code} error code is not recognised!", context={
                                   "show_id": show, "uid": uid})
        resp = flask.Response(fg.rss_str(pretty=True))
        resp.headers['Content-Type'] = 'application/rss+xml; charset=utf-8'
        return resp
        ...
        honeybadger.notify(
            f"{request_result.status_code} error code is not recognised!", context={"uid": uid})
        return "501", 501
